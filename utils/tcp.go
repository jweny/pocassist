...
func TcpSend(targetAddr string, data []byte) ([]byte, error) {
	tcpTimeout := time.Duration(basic.GlobalConfig.HttpConfig.DailTimeout) * time.Second
...
//	tcpTimeout := time.Duration(basic.GlobalConfig.HttpConfig.HttpTimeout) * time.Second
//	conn, err := net.DialTimeout("tcp", targetAddr, tcpTimeout)
//	// add tls
